...
class Endpoint {
    constructor(name, resource) {
        ...
        this.path_params = resource.path_params ?? null
        this.query_params = resource.query_params ?? null
        this.required_params = resource.required_params ?? null
        this.note = resource.note ?? null
    }

    getURI(paramBag) {
        let uri = this.url;
        for (const [param_name, param_info] of Object.entries(this.path_params)) {
            let value = paramBag[param_name] ?? param_info.default;
            if (param_info.type === 'bool') value = value ? param_info.True : param_info.False;
            if (param_info.leading_slash) value = '/' + value;
            if (param_info.trailing_slash) value += '/';

            if (implies(param_info.required, value !== undefined)) {
                uri = uri.replaceAll('{' + param_name + '}', value ?? '')
            }
            else {
                throw new Error(`Missing parameter [${param_name}] for endpoint [${this.name}]`)
            }
        }
        return uri;
    }
    getQuery(paramBag) {
        ...
        return new URLSearchParams(paramBag);
    }

    getURL(paramBag) {
        return this.getURI(paramBag) + '?' + this.getQuery(paramBag).toString();
    }
}
...
function findEndpoint(name) {
    if (RESOURCES.hasOwnProperty(name))
        return new Endpoint(name, RESOURCES[name]);
}

module.exports = { find: findEndpoint };